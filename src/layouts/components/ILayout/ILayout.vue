 ...
 * @param routes 路由列表
 */
function getAccessRoutes(routes: IRouter[]) {
  return routes.filter((route) => {
    if (route.children) {
      route.children = getAccessRoutes(route.children)
    }
    return checkAccess(userStore.userInfo, route.meta?.role || ACCESS_ENUM.USER)
  })
}
    ...
    <a-layout style="flex-direction: column">
      ...
      <a-layout style="gap: 1px">
        ...
        <a-layout class="i-layout__inner">
          ...
          <a-layout-footer>
            <div class="title">{{ $t('copyright.title') }}</div>
            <div class="version">{{ $t('copyright.version') }}</div>
          </a-layout-footer>
        </a-layout>
      </a-layout>
    </a-layout>
